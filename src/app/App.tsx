...
type Registration = { name: string; email: string; password: string; passwordConfirmation: string; cpf: string; document: string; phone: string; address: string; entity: string; oab: string; section: string; company: string; professional: string; professionalEmail: string; terms: boolean; messages: boolean };
...
const emptyRegistration: Registration = { name: "", email: "", password: "", passwordConfirmation: "", cpf: "", document: "", phone: "", address: "", entity: "", oab: "", section: "", company: "", professional: "", professionalEmail: "", terms: false, messages: false };
...
function Shell({ eyebrow, title, children, back, aside }: { eyebrow: string; title: ReactNode; children: ReactNode; back?: () => void; aside?: ReactNode }) {
  return (
...
export default function App() {
  ...
  const steps = useMemo(() => role === "escritorio" ? ["Perfil e contato", "Dados do escritório", "Equipe", "Segurança e consentimento"] : ["Perfil e contato", "Dados cadastrais", "Segurança e consentimento"], [role]);
  ...
  const handleAuthenticated = (email: string) => {
    const parsedName = email.includes("@") ? email.split("@")[0] : email;
    const formattedName = parsedName ? parsedName.charAt(0).toUpperCase() + parsedName.slice(1) : "Usuario";
    setIsAuthenticated(true);
    setActiveSession({ name: formattedName, roleLabel: "Escritorio" });
  };
  const handleLogout = () => {
  ...
  }, [isAuthenticated, activeSession, view]);
...
function Onboarding({ go, session }: { go: (view: View) => void; session: { name: string; roleLabel: string } }) {
  const actions = [
  ...
  return <Shell eyebrow="Primeiro acesso e onboarding" title="Como deseja começar?" aside={<div className="relative"><p className="font-mono text-xs uppercase tracking-[.14em] text-[#aebfff]">Conta ativa</p><div className="mt-4 rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-xs text-[#d8e4ff]">Usuario: {session.name} · Perfil: {session.roleLabel}</div><div className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-5"><div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-xl bg-[#32c700] text-[#143671]"><Check size={22}/></div><p className="text-sm leading-6 text-[#c6d2ff]">Seu espaco esta pronto para comecar.</p></div></div></div>}><div className="rounded-2xl border border-border bg-card p-4"><div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground"><span>Inicie com uma das opcoes abaixo, ou</span><Button secondary onClick={() => go("dashboard")}>Quero ir para o painel</Button></div></div><div className="mt-4 grid gap-3">{actions.map(([Icon, title, text, destination]) => <button key={title} onClick={() => go(destination)} className="group flex min-h-[76px] items-center gap-4 rounded-2xl border border-border p-4 text-left transition hover:border-[#9cb4ff] hover:bg-[#f8faff] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#7c3aed] motion-reduce:transition-none"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-[#eef3ff] text-[#2f63e5]"><Icon size={21}/></span><span className="min-w-0 flex-1"><span className="block font-semibold">{title}</span><span className="mt-0.5 block text-sm text-muted-foreground">{text}</span></span><ChevronRight size={20} className="text-muted-foreground group-hover:text-[#2f63e5]"/></button>)}</div></Shell>;
}
...
function AcceptInvitation({ go, notify }: { go: (view: View) => void; notify: (m: string) => void }) {
  ...
  return <Shell eyebrow="Convite recebido" title="Concluir cadastro para acessar" back={() => go("login")}><form className="space-y-5" onSubmit={event => { event.preventDefault(); if (!weak && !mismatch) { notify("Convite aceito com sucesso."); go("onboarding"); } }}><Notice title="Dados do convite bloqueados">O e-mail convidado e o identificador do convite são preenchidos automaticamente e não podem ser alterados.</Notice><Field id="accept-token" label="Código do convite" value="INV-BF-2026-001" onChange={() => undefined}/><Field id="accept-email" label="E-mail convidado" type="email" value="cliente@exemplo.com" onChange={() => undefined}/><Field id="accept-name" label="Seu nome" value={name} onChange={setName} required/><Field id="accept-password" label="Crie uma senha" type="password" value={password} onChange={setPassword} error={weak ? "Sua senha ainda está fraca. Use 10 ou mais caracteres." : ""} required/><Field id="accept-confirm" label="Confirmar senha" type="password" value={confirm} onChange={setConfirm} error={mismatch ? "As senhas não coincidem." : ""} required/><Button type="submit" className="w-full" disabled={!name || !password || !confirm || weak || mismatch}>Aceitar convite<Check size={18}/></Button></form></Shell>;
}
...
function Dashboard({ id, go, notify }: { id: string; go: (view: View) => void; notify: (m: string) => void }) {
  ...
  const cardData = {
    processos: {
      ...
      meta: "+2 esta semana",
      items: [
        "TRB-2026-0019 � Revis�o de inicial trabalhista",
        "LIC-2026-0071 � Acompanhamento de licita��o",
        "FAM-2026-0112 � Invent�rio em andamento",
        "IMO-2026-0099 � Regulariza��o de im�vel",
      ],
    },
    convites: {
